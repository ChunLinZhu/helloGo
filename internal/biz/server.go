// Package biz — Biz Service gRPC 服务器
package biz
...
// Server 管理 Biz Service gRPC 服务器生命周期
type Server struct {
...
// NewServer 创建 Biz Service gRPC 服务器
func NewServer(port int, svc *Service, logger *zap.Logger) *Server {
	...
	// 注册 BizService
	bizv1.RegisterBizServiceServer(grpcServer, svc)
...
func (s *Server) Run() error {
	...
	s.logger.Info("Biz Service gRPC 服务器已启动", zap.Int("port", s.port))
